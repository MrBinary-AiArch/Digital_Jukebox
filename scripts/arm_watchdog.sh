...
# Mark any completed/failed jobs as 'ejected' if they have been finished for more than 10 minutes.
# This prevents "Ghost Jobs" (like the notorious Job 333) from blocking the next disc insertion.
echo "[$(date)] Running Database Sanity Check..."
GHOST_JOBS=$(sqlite3 "$DB_PATH" "SELECT job_id FROM job WHERE ejected=0 AND status IN ('fail', 'success', 'completed') AND stop_time < datetime('now', '-10 minutes');")
if [ -z "$GHOST_JOBS" ]; then
    echo "  - No ghost jobs found."
else
    echo "  - Found ghost jobs: $GHOST_JOBS. Marking as ejected..."
    sqlite3 "$DB_PATH" "UPDATE job SET ejected=1 WHERE ejected=0 AND status IN ('fail', 'success', 'completed') AND stop_time < datetime('now', '-10 minutes');"
fi
